...
export const getWorkflowExecutionStats =  async (period: Period) => {
    ...
    executionsWithInRange.forEach(exec => {
        const date = format(exec.startedAt!, dateFormat)
        if(exec.status === WorkflowExecutionStatus.COMPLETED){ stats[date].success ++ } 
        if(exec.status === WorkflowExecutionStatus.FAILED){ stats[date].failed ++ } 
    })
